...
create table public.oportunidades (
  ...
  estado text not null default 'nueva'
    check (estado in ('nueva','contactada','propuesta','ganada','perdida')),
  valor_estimado numeric(12,2)
);

alter table public.oportunidades enable row level security;

-- La web pública (anon) solo puede CREAR oportunidades nuevas, nunca leerlas.
create policy "web crea oportunidades" on public.oportunidades
  for insert to anon, authenticated
  with check (estado = 'nueva' and valor_estimado is null);

-- El CRM (usuarios autenticados) puede leer y actualizar.
create policy "crm lee oportunidades" on public.oportunidades
  for select to authenticated using (true);
create policy "crm actualiza oportunidades" on public.oportunidades
  for update to authenticated using (true) with check (true);

-- Actualización en tiempo real en el CRM
alter publication supabase_realtime add table public.oportunidades;

-- ===== Ampliación: empleados y empresas =====
